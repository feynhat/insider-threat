...
\begin{frame}{The Isolation Forest Algorithm}
  ...
  \begin{algorithm}[H]
\caption{iTree($X'$)}\label{alg:cap}
\begin{algorithmic}[1]
  \Require $X'$, Input data
  \Ensure A tree
  \If{$X'$ cannot be divided}
  \State \Return \texttt{exNode}($X'$)
  \EndIf
  \State $Q \gets \text{ list of attributes in } X'$
\State randomly select an attribute $q \in Q$
\State randomly select a split point $p$ between the \texttt{max} and \texttt{min} values of
the attribute $q$ in $X'$
\State $X_l \gets $ \texttt{filter}$(X, q \le p)$
\State $X_r \gets $ \texttt{filter}$(X, q > p)$
\State \Return \texttt{inNode(iTree(}$X_l$\texttt{), iTree(}$X_r$\texttt{))}
\end{algorithmic}
\end{algorithm}
...
\begin{frame}{The Isolation Forest Algorithm}
  ...
  \begin{algorithm}[H]
\caption{iForest($X$, $t$, $\psi$)}\label{alg2:cap}
\begin{algorithmic}[1]
  \Require $X$ data, $t$ number of trees, $\psi$ subsampling size
  \Ensure A set of \texttt{iTree}'s
  \State Initialize \texttt{Forest}
  \For{$i = 1$ to t}
  \State $X' \gets $ \texttt{sample}($X, \psi$)
  \State \texttt{Forest} $\gets$ \texttt{Forest} $\cup$ \texttt{iTree}($X'$)
  \EndFor
  \State \Return \texttt{Forest}
\end{algorithmic}
\end{algorithm}
\pause
\begin{itemize}
  \item Time complexity: $\mathcal{O}(t\psi^2)$
  \item Space complexity: $\mathcal{O}(t\psi)$
\end{itemize}
